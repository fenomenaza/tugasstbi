...
$query = "INSERT INTO `tbindex`(`Term`, `DocId`, `Count`) SELECT `token`,`nama_file`,count(*) FROM `dokumen` group by `nama_file`,token";
...
//berapa jumlah DocId total?, n
	...
	$query = "SELECT * FROM tbindex ORDER BY Id";
	...
	while($rowbobot = mysqli_fetch_array($resBobot)) {
		...
		$query = "SELECT Count(*) as N FROM tbindex WHERE Term = '$term'";
